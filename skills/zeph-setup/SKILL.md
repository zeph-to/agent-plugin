...
2. If not set, guide them:
   ...
   - Create a key with "MCP" preset (includes push:read, push:write, hook:write, channel:read)
   - Copy the key (starts with ak_...)

3. Ask if they want interactive features (prompt/input):
   - If yes: Go to Settings → Developer → Hooks → Create Hook
